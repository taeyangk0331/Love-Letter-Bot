...
def run_discord_bot():
    ...
    async def on_message(message):
        ...
        if channel == 'loveletter':
            if user_message == 'join':
            ...
            elif user_message == 'start' and start == True:
                ...
                def guard_guess_player(m):
                    if int(m.content) < 10 or int(m.content) > 0:
                        return True
                ...
                if action == '1' or message.content.startswith('guard'): #not done
                    ...
                    for num in range(2):
                        print(player_dict_shuffled[guard_choose_player_.content][num] == '10')
                        print(type(player_dict_shuffled[guard_choose_player_.content][num]))
                        print(player_dict_shuffled[guard_choose_player_.content][num])
                        if player_dict_shuffled[guard_choose_player_.content][num] == 10:
                            await before.edit(content = (str(guard_choose_player_.content) + ' is protected\nget rekt bitch'))
                            break
                        else:
                        ...
                        for find_zero in range(2):
                            if player_dict_shuffled[prince_.content][find_zero] == 10:
                                ...
                                if player_dict_shuffled[prince_.content][find_zero] != 8:
                                    ...
                                    await message.channel.send(str(player_dict_shuffled[prince_.content][find_zero]) + '\n' + str(prince_.content) + ' has been eliminated')
                                    del player_dict_shuffled[prince_.content]
                                    player_pool -= 1
                                    break
                                    #eliminate
                    dump_pile.append(list(player_dict_shuffled.values())[turns][0])
                ...
                elif (action == '5' or message.content.startswith('prince')) and (list(player_dict_shuffled.values())[turns][0]) == 7:
                ...
                elif action == '6' or message.content.startswith('king') and (list(player_dict_shuffled.values())[turns][0]) == 7:
                    await message.channel.purge(limit=1)
                    await message.channel.send(content = "You are not allow to use either prince(5) or king(6) if you have countess(7) in your hand")
            ...
            elif (nickname == str(list(player_dict_shuffled.keys())[turns]) or (message.content.startswith(str(list(player_dict_shuffled.values())[turns][1])) or (user_message in str(card_word))) and guard_ability  == 0):
                action = str(list(player_dict_shuffled.values())[turns][1])
                ...
                if action == '1' or message.content.startswith('guard'): #not done
                    ...
                    if int(guard_guess_player_.content) in player_dict_shuffled[guard_choose_player_.content]:
                        ...
                        guard_ability = 0
                        dump_pile.append(list(player_dict_shuffled.values())[turns][1])
                        list(player_dict_shuffled.values())[turns][1] = 0
                        turns = await draw_card_func(player_dict_shuffled, shuffled_card, message, turns, player_pool)
                    elif int(guard_guess_player_.content) not in player_dict_shuffled[guard_choose_player_.content]:
                        print("debug False")
                        await message.channel.send(str(list(player_dict_shuffled.keys())[turns]) + " has guessed wrong card")
                    ...
                    await message.channel.send(str(list(player_dict_shuffled.keys())[turns] + " is protected until own turn"))
                    dump_pile.append(list(player_dict_shuffled.values())[turns][1])
                    list(player_dict_shuffled.values())[turns][1] = 10
                    turns = await draw_card_func(player_dict_shuffled, shuffled_card, message, turns, player_pool)
